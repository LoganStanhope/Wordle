...
#reader(lib "htdp-intermediate-lambda-reader.ss" "lang")((modname |hw 9 Logan Stanhope + Amanda Fong|) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
(define (value-in-list? x lox fun)
  (local [; value-in-list-helper : X -> Boolean
          ; are these two values equal to each other?
          (define (value-in-list-helper y)
            (fun x y))]
    (ormap value-in-list-helper lox)))
...
(define (read-dictionary str)
  (if (file-exists? str)
      (map string-upcase (read-lines str))
      '()))
...
(define (score str1 str2)
  (local [; score-helper : String String -> LetterStatusPair
          ; check to see if the guessed word is the correct word
          ; and then makes a pair based on if it is correct, wrong, or
          ; misplaced
          (define (score-helper guessed correct)
            (make-pair guessed (cond [(string=? guessed correct) LS-RIGHT]
                                     [(string-in-list? guessed EXPLODE2) LS-MISPLACED]
                                     [else LS-WRONG])))
          (define EXPLODE1 (explode str1))
          (define EXPLODE2 (explode str2))]
                                     
    (mymap2 EXPLODE1 EXPLODE2 score-helper)))
...
(define (partial-guess->full-guess str nat)
  (local [; exploded-str : String -> [List-of String]
          ; takes the given string and turns it into a list
          ; of one strings
          (define exploded-str (explode str))]
    (implode
     (append exploded-str (build-list (- nat (string-length str))
                                      (λ (x) " "))))))
...
(define (key-wordle ws akey los nat)
  (local [; backspace-helper : WS -> WS
          ; stops the user from typing a letter if the string length
          ; exceeds the given word length or allows the deletion of a letter
          (define (backspace-helper f)
            (if (= 0 (string-length (wordle-current f)))
                ws
                (make-wordle (wordle-past f)
                             (substring (wordle-current f)
                                        0 (sub1 (string-length (wordle-current f)))))))
          ; return-helper : WS -> WS
          ; finalizes an attempted guess in the game of wordle  
          (define (return-helper y)
            (if (and (= (string-length (wordle-current y)) nat)
                     (ormap (λ (x) (string=? x (wordle-current y))) los))
                (make-wordle (append (wordle-past ws) (list (wordle-current ws))) "")
                y))
                    
          ; letter-key-helper : WS -> WS
          ; allows the user to type a letter into the game of wordle
          (define (letter-key-helper x)
            (cond [(> (string-length (string-append (wordle-current x) akey)) nat) x]
                  [(number? (string->number akey)) x]
                  [else (make-wordle (wordle-past x) (string-append (wordle-current x)
                                                                    (string-upcase akey)))]))]
    (cond
      [(key=? akey "\b") (backspace-helper ws)]
      [(key=? akey "\r") (return-helper ws)]
      [else (letter-key-helper ws)])))
...
; guessed
(define (end-wordle? ws str nat)         
  (or (= (length (wordle-past ws)) nat)
      (ormap (λ (x) (string=? str x)) (wordle-past ws)))) 
...
(define BG (rectangle 500 500 "solid" BG-COLOR))

; draw-end-wordle : WS String Nat -> Image
; produces the game end screen
(check-expect (draw-end-wordle WS-START "bear" 1)
              (place-image
               (text "guesses: 0" 10 "purple") 100 100
               (place-image
                (text "maybe next time?" 20 "purple") 100 150
                BG)))

(check-expect (draw-end-wordle WS-WORD3 "" 3)
              (place-image
               (text "guesses: 3" 10 "purple") 100 100
               (place-image
                (text "YOU WON WOOO!!!!" 20 "purple") 100 150
                BG)))              
              
(define (draw-end-wordle ws str nat)
  (if (or (= (length (wordle-past ws)) nat)
          (ormap (λ (word) (string=? str word)) (wordle-past ws)))
      (place-image
       (text (string-append "guesses: "
                            (number->string (length (wordle-past ws)))) 10 "purple")
       100 100
       (place-image
        (text "YOU WON WOOO!!!!" 20 "purple") 100 150
        BG))
...
(check-expect (draw-wordle WS-START "   " 3)
              (stack/v (append
                        (image-maker/past '() "   ")
                        (list (partial-guess->image "" 3))
                        (image-maker/future 3 2))))

(check-expect (draw-wordle WS-WORD1 "ETA" 3)
              (stack/v (append
                        (image-maker/past (list "CAT") "ETA")
                        (list (partial-guess->image "" 3))
                        (image-maker/future 3 1))))
              
(define (draw-wordle ws string nat)
  (local [; remaining-guesses : WS Nat -> Nat
          ; determines how many guesses the player has left
          (define REMAINING-GUESSES (- nat (length (wordle-past ws))))
          
          ; current-guess : WS String -> [List-of Image]
          ; determines how many guesses the user had left exlcuding the one
          ; they're on
          (define CURRENT-GUESS (if (> REMAINING-GUESSES 0)
                                    (list (partial-guess->image (wordle-current ws)
                                                                (string-length string)))
                                    (list)))]
    
    (stack/v (append
              (image-maker/past (wordle-past ws) string)
              CURRENT-GUESS
              (image-maker/future (string-length string) (- REMAINING-GUESSES 1))))))

; image-maker/past : [List-of String] String -> [List-of Image]
; creates a list of images of the past guessed words

(check-expect (image-maker/past (list "G") "P")
              (list (beside
                     GAP
                     (lsp->image (make-pair "G" LS-WRONG))
                     GAP)))

(check-expect (image-maker/past (list "C") "C")
              (list (beside
                     GAP
                     (lsp->image (make-pair "C" LS-RIGHT))
                     GAP)))

(check-expect (image-maker/past (list "Q") "D")
              (list (beside
                     GAP
                     (lsp->image (make-pair "Q" LS-WRONG))
                     GAP)))

(define (image-maker/past los correctguess)
  (local
    [; String -> LetterStatusPair
     ; takes the correct guess and turns it into a pair
     (define l
       (map (λ (guess) (score guess correctguess)) los))]
    (map (λ (x) (row->image
                 (map (λ (lsp) (make-pair lsp lsp->image)) x))) l)))
                                        

; image-maker/future :  PosInt PosInt -> [List-of Image]
; creates a list of images of the future guesses available to the player
(check-expect (image-maker/future 4 2)
              (append (list
                       (beside GAP
                               (guess-letter->image " ")
                               GAP
...
(check-expect (image-maker/future 2 1)
              (append (list
                       (beside GAP
                               (guess-letter->image " ")
                               GAP
...
(define (image-maker/future str-length rem-guesses)
  (build-list rem-guesses (λ (x) (partial-guess->image "" str-length)))) 
